...
sub build_deb_filename {
	...
	$deb_file =
	  $pkg_name . "_" . $pkg_version_file . "+atom_" . $arch . ".deb";
}
...
sub build
{
    ...
    if ($r)
    {
        ...
        # Now build
        $r = !system "DEB_BUILD_OPTIONS=nocheck CXXFLAGS_EXTRA=\"-Os -mtune=bonnell -march=bonnell -mfpmath=sse\" DEB_CFLAGS=\"-Os -mtune=bonnell -march=bonnell -mfpmath=sse -fomit-frame-pointer\" DEB_CXXFLAGS=\" -Os -mtune=bonnell -march=bonnell -mfpmath=sse -fomit-frame-pointer\" dpkg-buildpackage -j8";
        wait;
    }
